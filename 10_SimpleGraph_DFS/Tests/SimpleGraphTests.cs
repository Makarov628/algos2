...
namespace AlgorithmDataStructures2Tests
{
    ...
    public class SimpleGraphTests
    {
        ...
        [Test]
        public void TestLongestSimplePathLength()
        {
            var directedGraph = new DirectedGraph(6);
            ...
            directedGraph.AddEdge(2, 5); // Добавляем дополнительный путь к вершине 5

            int longestPathLength = directedGraph.LongestSimplePathLength();
            Assert.That(longestPathLength, Is.EqualTo(5)); // Ожидаемая длина пути: 5

            // Добавляем цикл и проверяем, что метод игнорирует его
            directedGraph.AddEdge(4, 1); // Цикл: 1 -> 2 -> 3 -> 4 -> 1
            longestPathLength = directedGraph.LongestSimplePathLength();
            Assert.That(longestPathLength, Is.EqualTo(5)); 
        }
